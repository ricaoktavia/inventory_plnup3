...
async function run() {
    ...
    // Current stocks (UP3)
    const [currentStocks] = await conn.query('SELECT material_id, quantity FROM stocks WHERE ulp_id IS NULL');
    ...
    periodRows.forEach(r => periodMap.set(r.materialId, { incoming: Number(r.incoming||0), outgoing: Number(r.outgoing||0) }));
    const afterMap = new Map();
    afterRows.forEach(r => afterMap.set(r.materialId, { incoming: Number(r.incoming||0), outgoing: Number(r.outgoing||0) }));
    ...
    fs.writeFileSync('d:/inventory_plnup3/scratch/verify_fix.json', JSON.stringify({
        totalRows,
        nonZeroAwal,
