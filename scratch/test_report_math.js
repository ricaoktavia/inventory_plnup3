...
async function run() {
    ...
    const startDateStr = new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().split('T')[0];
    ...
    fs.writeFileSync('d:/inventory_plnup3/scratch/report_math_out.json', JSON.stringify(output, null, 2), 'utf-8');
    console.log('Done');
    process.exit(0);
}
