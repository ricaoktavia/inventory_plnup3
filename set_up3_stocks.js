...
async function run() {
    ...
    for (const mat of materials) {
        const [existing] = await conn.query('SELECT id FROM stocks WHERE material_id = ? AND ulp_id IS NULL', [mat.id]);
        if (existing.length > 0) {
            await conn.query('UPDATE stocks SET quantity = 500 WHERE id = ?', [existing[0].id]);
        } else {
            await conn.query('INSERT INTO stocks (material_id, ulp_id, quantity) VALUES (?, NULL, 500)', [mat.id]);
        }
    }
