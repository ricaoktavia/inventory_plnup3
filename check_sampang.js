...
async function run() {
	...
	if (sampang.length > 0) {
		const [rows] = await conn.query('SELECT * FROM stocks WHERE ulp_id = ? AND quantity >= 10', [
			sampang[0].id
		]);
	...
	const [dupes] = await conn.query(
		'SELECT material_id, ulp_id, COUNT(*) as cnt FROM stocks GROUP BY material_id, ulp_id HAVING cnt > 1'
	);
