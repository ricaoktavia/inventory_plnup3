...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const criticalStocksQuery = await db
		.select({
	...
	const maxTop5Value =
		top5Query.length > 0 ? Math.max(...top5Query.map((t) => Number(t.total))) : 1;
