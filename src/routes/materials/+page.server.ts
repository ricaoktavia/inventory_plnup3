...
export const load: PageServerLoad = async ({ locals, url }) => {
	...
	const selectedUlpId =
		...
		(user?.role === 'ADMIN_UP3' ? 'rekap' : user?.ulpId?.toString());
...
export const actions: Actions = {
	tambah: async ({ request, locals }) => {
		...
		if (user?.role !== 'ADMIN_UP3')
			return fail(403, { error: 'Hanya Admin UP3 yang dapat menambah material.' });
	...
	edit: async ({ request, locals }) => {
		const user = locals.user;
		if (user?.role !== 'ADMIN_UP3')
			return fail(403, { error: 'Hanya Admin UP3 yang dapat mengedit.' });
		...
		if (!materialId || !name || !unit || quantityStr === undefined)
			return fail(400, { error: 'Data edit tidak valid.' });
		...
		if (existingStock) {
			await db.update(stocks).set({ quantity }).where(eq(stocks.id, existingStock.id));
		} else {
			await db.insert(stocks).values({
				materialId,
				ulpId: targetUlpIdVal,
	...
	hapus: async ({ request, locals }) => {
		const user = locals.user;
		if (user?.role !== 'ADMIN_UP3')
			return fail(403, { error: 'Hanya Admin UP3 yang dapat menghapus.' });
		...
		try {
			...
			return fail(400, {
				error: 'Gagal dihapus: Material ini masih terikat pada riwayat Transaksi mutasi.'
			});
		}
	}
};
